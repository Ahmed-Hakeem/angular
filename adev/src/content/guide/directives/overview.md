...
For this use case, Angular applies the classes on initialization and in case of changes happened by reassigning the `currentClasses` object.
The full example calls `setCurrentClasses()` initially with `ngOnInit()` and when the user click on `Refresh currentClasses` button.
